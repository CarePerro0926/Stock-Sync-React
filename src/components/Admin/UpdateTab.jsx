...
const UpdateTab = ({ productos = [], categorias = [], proveedores = [], onUpdateSuccess, onUpdateProveedorSuccess }) => {
  ...
  const [formProvData, setFormProvData] = useState({ nombre: '', email: '', telefono: '', direccion: '' });

  const getListaProveedores = () => {
    if (!proveedores || !Array.isArray(proveedores)) return [];
    return proveedores.map(pr => ({ id: pr.id, nombre: pr.nombre, email: pr.email || '' }));
  };
  ...
  const handleBuscarProveedor = (e) => {
    ...
    if (!encontrado && proveedorSeleccionado) {
      encontrado = (proveedores || []).find(pr => String(pr.id) === String(proveedorSeleccionado));
    }
  ...
  const handleSubmitProveedor = async (e) => {
    ...
    if (email && !emailRegex.test(email)) { alert('Email inválido.'); return; }
    if (telefono && !/^[0-9+\-\s()]{7,}$/.test(telefono)) { alert('Teléfono inválido.'); return; }
    ...
    if (onUpdateProveedorSuccess) { try { onUpdateProveedorSuccess(); } catch (e) { console.error(e); } }
    ...
    <>
      ...
      <form onSubmit={handleBuscar} className="mb-4">
        ...
        <div className="mb-3">
          ...
          <select className="form-select" value={productoSeleccionado} onChange={(e) => {
            const val = e.target.value;
            setProductoSeleccionado(val);
            const encontrado = (productos || []).find(p => String(p.id) === String(val));
            if (encontrado) {
              setProducto(encontrado);
              setFormData({ nombre: encontrado.nombre || '', precio: encontrado.precio ?? '', cantidad: encontrado.cantidad ?? '', categoria: encontrado.categoria_id ?? '' });
            } else {
              setProducto(null);
              setFormData({ nombre: '', precio: '', cantidad: '', categoria: '' });
            }
      ...
      <form onSubmit={handleBuscarProveedor} className="mb-4">
            ...
            {busquedaProv && sugerenciasProv.length > 0 && (
              <ul className="list-group position-absolute z-3 w-100">
                {sugerenciasProv.map(pr => (
                  <li key={pr.id} className="list-group-item list-group-item-action" style={{ cursor: 'pointer' }}
                    onClick={() => {
                      setProveedor(pr);
                      setFormProvData({ nombre: pr.nombre || '', email: pr.email || '', telefono: pr.telefono || '', direccion: pr.direccion || '' });
        ...
        <div className="mb-3">
          ...
          <select className="form-select" value={proveedorSeleccionado} onChange={(e) => {
            const val = e.target.value;
            setProveedorSeleccionado(val);
            const encontrado = (proveedores || []).find(p => String(p.id) === String(val));
            if (encontrado) {
              setProveedor(encontrado);
              setFormProvData({ nombre: encontrado.nombre || '', email: encontrado.email || '', telefono: encontrado.telefono || '', direccion: encontrado.direccion || '' });
            } else {
              setProveedor(null);
              setFormProvData({ nombre: '', email: '', telefono: '', direccion: '' });
            }
            ...
            <option value="">—</option>
            {listaProveedores.map(pr => (<option key={pr.id} value={pr.id}>{pr.id} - {pr.nombre}{pr.email ? ` - ${pr.email}` : ''}</option>))}
          </select>
        </div>
      </form>
        ...
        <form onSubmit={handleSubmitProveedor}>
          <div className="row g-2 mb-3">
            <div className="col-12 col-md-6">
              ...
              <input name="nombre" className="form-control" value={formProvData.nombre} onChange={handleProvChange} required />
            </div>
            <div className="col-12 col-md-6">
              <label className="form-label">Email</label>
              <input name="email" type="email" className="form-control" value={formProvData.email} onChange={handleProvChange} />
            </div>
            <div className="col-12 col-md-6">
              <label className="form-label">Teléfono</label>
              <input name="telefono" className="form-control" value={formProvData.telefono} onChange={handleProvChange} />
            </div>
            <div className="col-12 col-md-6">
              <label className="form-label">Dirección</label>
              <input name="direccion" className="form-control" value={formProvData.direccion} onChange={handleProvChange} />
            </div>
          </div>
